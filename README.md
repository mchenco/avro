...
1. [Make sure you have python installed](https://docs.python.org/3.8/faq/windows.html)
	...
	- If you don't ... [try this](https://www.liquidweb.com/kb/how-to-install-python-on-windows/)
2. [Download pip](https://www.liquidweb.com/kb/install-pip-windows/)
	- Download this [pip installer](https://bootstrap.pypa.io/get-pip.py)
	- Open Command Prompt, navigate to the folder that you downloaded the pip installer to
	- Type `python get-pip.py` in Command Prompt
	- It should download lol idk you can type `pip -v` into the terminal to check
...
1. Make sure your folder that has the script has the following things in it:
	...
	- Your two data files: 2-24h and 48h
	- `script.py`
2. In terminal, type `python script.py`
	- The screen will prompt you to enter the filenames of your data files. I put it this way so it's not too limiting in case your files are named differently each time
	- **Don't include the `.xlsx`ext when entering filename**
	- The script will run and output your 3 files to a folder  with today's date on it in your current directory
3. ENJOY :)
